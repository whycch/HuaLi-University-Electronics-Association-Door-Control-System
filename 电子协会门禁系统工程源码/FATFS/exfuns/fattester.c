...
u8 OPEN_DOOR(u8*c)   //用串口发送这个函数并加上字符串可以实现蓝牙串口开门
{                    //在串口软件中输入OPEN_DOOR("I Love Electronic Association");    (注意)记得在串口软件中勾选换新行发送，
	if(c=="I Love Electronic Association");                                           //或者在函数的";"分号后面加键盘的换行键(ENTER)
	{
		Open_Door_flag=3;
		LED0=0;
	}
